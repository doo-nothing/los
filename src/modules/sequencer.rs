...
struct SequencerState {
    ...
    /// Added to every note event's `source` so receivers can tell
    /// sequencer instances apart: instance N's tracks are sources
    /// N*8 … N*8+7 (routing::note_source_track mirrors this).
    src_base: u8,
...
fn apply_action(s: &mut SequencerState, h: &mut History, action: &Action) -> Option<String> {
    ...
    match action {
        ...
        Action::Adjust {
            ...
            let period = s.tracks[tidx]
                .scale
                .as_ref()
                .map_or(12, |sc| sc.len() as i32);
            ...
            match layer {
                ...
                state::BindTarget::Repeats => {
                    ...
                    st.repeats = (i32::from(st.repeats) + d).clamp(1, i32::from(MAX_REPEATS)) as u8;
                }
        ...
        Action::Op { op, motion, count } => {
            let Some((a, b)) = motion_range(&s.tracks[tidx], s.selected, *motion, (*count).max(1))
            else {
        ...
        Action::Euclid(op) => {
            ...
            match op {
                ...
                EuclidOp::RotatePlus(n) => {
                    s.tracks[tidx].rotation = (s.tracks[tidx].rotation + n) % s.tracks[tidx].length;
                }
                EuclidOp::Reapply => {
            ...
            let (p, l, r) = (
                s.tracks[tidx].pulses,
                s.tracks[tidx].length,
        ...
        Action::SetDelayUnit { unit, convert } => {
            ...
            if *convert {
                ...
                st.delay = match unit {
                    state::DelayUnit::Ms => (st.delay / 100.0 * step_ms).clamp(0.0, MAX_DELAY_MS),
                    state::DelayUnit::Pct => (st.delay / step_ms * 100.0).clamp(0.0, MAX_DELAY_PCT),
                };
            }
            st.delay_unit = *unit;
            let new_step = s.tracks[tidx].steps[sel].clone();
...
fn fmt_macro_cmd(cmd: &state::MacroCmd) -> String {
    match cmd {
        ...
        state::MacroCmd::SetMute {
            track,
            muted: false,
        } => format!("unmute {}", track + 1),
    ...
    for seg in input.split('|') {
        ...
        match words.as_slice() {
            ...
            ["pat", t, slot] => {
                ...
                cmds.push(state::MacroCmd::SwitchPattern {
                    track: parse_track(t)?,
                    slot,
                });
            }
            ["mute", t] => cmds.push(state::MacroCmd::SetMute {
                track: parse_track(t)?,
                muted: true,
            }),
            ["unmute", t] => cmds.push(state::MacroCmd::SetMute {
                track: parse_track(t)?,
                muted: false,
            }),
            ["cycle", t, m] => {
                let mode = state::CycleMode::parse(m).ok_or_else(|| format!("bad cycle: {m}"))?;
                cmds.push(state::MacroCmd::SetCycle {
                    track: parse_track(t)?,
                    mode,
                });
            }
            ["xpose", t, n] => {
                ...
                cmds.push(state::MacroCmd::TransposeTrack {
                    track: parse_track(t)?,
                    by,
                });
            }
            ["rot", t, n] => {
                let by: i32 = n.parse().map_err(|_| format!("bad amount: {n}"))?;
                cmds.push(state::MacroCmd::RotateTrack {
                    track: parse_track(t)?,
                    by,
                });
            }
            ["scale", t, rest @ ..] if !rest.is_empty() => {
                ...
                cmds.push(state::MacroCmd::SetScale {
                    track: parse_track(t)?,
                    scale,
                });
            }
            ...
            ["quant", q] => {
                quant = Some(state::Quant::parse(q).ok_or_else(|| format!("bad quant: {q}"))?);
            }
        ...
        match self {
            ...
            Command::EditSteps {
                ...
                if *track < state.tracks.len()
                    && start + old.len() <= state.tracks[*track].steps.len()
                {
    ...
    fn redo(&self, state: &mut SequencerState) {
        match self {
            ...
            Command::EditSteps {
                ...
                if *track < state.tracks.len()
                    && start + new.len() <= state.tracks[*track].steps.len()
                {
...
fn macro_cmds_of(cmd: &Command) -> Vec<state::MacroCmd> {
    match cmd {
        ...
        Command::SetCycle { track, new, .. } => {
            vec![state::MacroCmd::SetCycle {
                track: *track,
                mode: *new,
            }]
        }
        Command::SwitchSlot { track, to, .. } => {
            vec![state::MacroCmd::SwitchPattern {
                track: *track,
                slot: *to,
            }]
        }
...
fn apply_macro_cmd(s: &mut SequencerState, cmd: &state::MacroCmd, seed: u64) -> Option<Command> {
    match cmd {
        ...
        state::MacroCmd::SetSteps {
            ...
            let new: Vec<Step> = steps[..=(end - start)]
                .iter()
                .map(step_from_param)
                .collect();
            if old == new {
...
fn fire_macro_now(s: &mut SequencerState, idx: usize, seed: u64) {
    ...
    match undo.len() {
        ...
        _ => s.macro_outbox.push(Command::Group {
            cmds: undo,
            desc: "Macro",
        }),
    }
...
fn quant_boundary(quant: state::Quant, gstep: u64, first_track_len: Option<usize>) -> u64 {
...
fn seq_completer(macro_ids: Vec<String>) -> impl Fn(&str, &str) -> Vec<String> {
    move |head: &str, word: &str| {
        let items: Vec<String> = match head {
            "" => [
                "w", "e", "q", "q!", "wq", "x", "set", "scale", "fill", "macro", "swing", "groove",
                "humanize", "decay",
            ]
            .iter()
            .map(|s| s.to_string())
...
fn start_preview(state: &Mutex<SequencerState>, line: &str, fill_seed: u64) -> Option<History> {
    ...
    let applied = match crate::excmd::parse(line) {
        ...
        ExCommand::Unknown(c) => {
            ...
            match head {
                ...
                "decay" if !rest.is_empty() => rest.parse::<i8>().ok().map(|v| {
        ...
        // Scope the lock so it's not held during sleep
        {
            ...
            if playing && last_gstep != Some(gstep) {
                ...
                for t in 0..s.tracks.len() {
                    ...
                    let pos =
                        cycle_pos(s.tracks[t].cycle, gstep, len, &mut drunk[t], track_seed(t))
                    ...
                    for pf in pending_fires.drain(..) {
                        ...
                        match pf.fire {
                            ...
                            Fire::BusSet { track, value } => {
                                if let (Some(ref mut bus), Some(mb)) = (modbus.as_mut(), s.mod_base)
                                {
                                    if track < s.tracks.len() && !s.tracks[track].muted {
...
fn advance_phase(phase: Option<f64>, clock: u64, last_clock: u64, samples_per_step: u64) -> f64 {
    match phase {
        Some(p) if clock >= last_clock => p + (clock - last_clock) as f64 / samples_per_step as f64,
...
fn step_fires(prob: u8, track: usize, gstep: u64) -> bool {
    ...
    let mut r =
        crate::theory::rng::Rng::new(track_seed(track) ^ gstep.wrapping_mul(0xBF58_476D_1CE4_E5B9));
...
fn fire_offset(
    ...
    let base = swing_frac(track.swing, gstep)
        + f64::from(crate::theory::groove::offset(
            track.groove.as_deref(),
            gstep,
        ))
...
fn stuck_notes(tracks: &[Track], last_notes: &[Option<u8>], playing: bool) -> Vec<(usize, u8)> {
    last_notes
        ...
        .filter(|(t, _)| !playing || tracks[*t].muted || tracks[*t].mode == TrackMode::Modulation)
        .collect()
}
...
fn draw_ui(
    ...
    terminal.draw(|f| {
        ...
        let vline_span = (mode == "visual_line").then(|| {
        ...
        for (ti, trk) in state.tracks.iter().enumerate() {
            ...
            spans.push(Span::styled(
                info_text,
                if is_cur { row_style } else { theme::dim() },
            ));
            lines.push(Line::from(spans));
        }
        ...
        for i in first..(first + visible).min(trk.length) {
            ...
            if let Some((b, &v)) = live {
                ...
                let text = match b.target {
                    ...
                    state::BindTarget::Mod => {
                        format!("{:+.2}", (step.mod_value + off.mod_value).clamp(-1.0, 1.0))
                    }
                    state::BindTarget::Delay => {
                    ...
                    state::BindTarget::Repeats => format!(
                        "x{}→x{}",
                        step.repeats,
                        (i32::from(step.repeats) + off.repeats).clamp(1, i32::from(MAX_REPEATS))
                    ),
...
fn layer_cell(layer: state::BindTarget, trk: &Track, step: &Step) -> (char, ratatui::style::Color) {
    use crate::theme;
    ...
    match layer {
        ...
        state::BindTarget::Velocity => {
            if step.active {
                (
                    theme::meter_char(f32::from(step.velocity) / 127.0),
                    theme::note(),
                )
            } else {
        ...
        state::BindTarget::DelayProb => {
            if step.active || trk.mode == TrackMode::Modulation {
                ...
                let color = if step.delay_prob < 100 {
                    theme::clock()
                } else {
                    theme::note()
                };
        ...
        state::BindTarget::RepeatProb => {
            if step.active && trk.mode == TrackMode::Note {
                ...
                let color = if step.repeat_prob < 100 {
                    theme::clock()
                } else {
                    theme::note()
                };
...
fn row_info(trk: &Track) -> String {
    ...
    format!(
        ...
        scale,
        timing,
        if trk.mode == TrackMode::Modulation {
            " ⌁"
        } else {
            ""
        },
...
fn snapshot_params(s: &SequencerState) -> state::SequencerParams {
    state::SequencerParams {
        ...
        lane: s
            ...
            .iter()
            .map(|slot| {
                slot.map(|i| ((b'a' + i as u8) as char).to_string())
                    .unwrap_or_default()
            })
            .collect(),
...
fn scale_from_params(tp: &state::TrackParam) -> Option<crate::theory::scales::Scale> {
    if !tp.scale_cents.is_empty() {
        ...
        let valid = period.is_finite()
            ...
            && degrees
                .iter()
                .all(|d| d.is_finite() && *d >= 0.0 && *d < period)
            && degrees[0].abs() < 1e-9
...
pub fn run(instance: usize) -> Result<()> {
    ...
    state.lock().unwrap().src_base = (instance * crate::NUM_TRACKS).min(255) as u8;
    ...
    if let Ok(params) = state::load_module_state::<state::SequencerParams>("sequencer", instance) {
        ...
        if !params.tracks.is_empty() {
            ...
            *trk = Track::empty();
            if let Some(p) = params.euclidean_pulses {
                trk.pulses = p.min(NUM_STEPS);
            }
            if let Some(l) = params.euclidean_length {
                trk.length = l.clamp(1, NUM_STEPS);
            }
            if let Some(r) = params.euclidean_rotation {
                trk.rotation = r.min(255);
            }
    ...
    let mut baseline =
        state::to_toml_string(&patch_params(&state.lock().unwrap())).unwrap_or_default();
    ...
    loop {
        ...
        if state::check_reload_signal() {
            if let Ok(params) =
                state::load_module_state::<state::SequencerParams>("sequencer", instance)
            {
                let mut s = state.lock().unwrap();
                if let Some(bpm) = params.bpm {
                    if bpm.is_finite() {
        ...
        let status_msg = if ex.is_active() {
            Some(ex.display())
        } else {
            undo_msg.clone()
        };
                    ...
                    MouseEventKind::Down(MouseButton::Left) => {
                        ...
                        if y == 1 {
                            ...
                            if lane_len > 0 {
                                ...
                                let info_len = format!("  {:>3} lane", lane_len).chars().count();
                                let visible = row_visible(w, info_len, lane_len);
                                let start = row_window_start(lane_len, s.lane_selected, visible);
            ...
            if let Event::Key(key) = ev {
                ...
                if ex.is_active() {
                    ...
                    }
                    {
                        ...
                        match cmd {
                            ...
                            ExCommand::Edit(name) => {
                                match state::load_patch::<state::SequencerParams>(&name) {
                                    Ok(p) => {
                                        let mut s = state.lock().unwrap();
                                        ...
                                        baseline = state::to_toml_string(&patch_params(&s))
                                            .unwrap_or_default();
                                        drop(s);
                                        history = History::new();
                                ...
                                match k.as_str() {
                                    ...
                                    "pulses" | "length" | "rotation" => match v.parse::<usize>() {
                                        Ok(n) => {
                                            ...
                                            match k.as_str() {
                                                "pulses" => {
                                                    s.tracks[tidx].pulses = n.min(NUM_STEPS)
                                                }
                                                "length" => {
                                                    s.tracks[tidx].length = n.clamp(1, NUM_STEPS);
                                                    let len = s.tracks[tidx].length;
                            ...
                            ExCommand::Unknown(c) => {
                                ...
                                match head {
                                    "scale" => {
                                        ...
                                        if rest.is_empty() {
                                            ...
                                            undo_msg = Some(match parse_root(root) {
                                                Some(r) => {
                                                    set_scale(&mut s, &mut history, None, Some(r))
                                                }
                                                None => format!("Invalid root: {}", root),
                                            });
                                        } else if rest.ends_with(".scl") {
                                            undo_msg = Some(
                                                match crate::theory::scl::load_scl(
                                    ...
                                    "groove" => {
                                        ...
                                        let parsed: Option<Option<String>> = if name.is_empty()
                                            || name == "straight"
                                            || name == "off"
                                        {
                                            Some(None)
                                        ...
                                        match parsed {
                                            Some(g) => {
                                                let label = g
                                                    .clone()
                                                    .unwrap_or_else(|| String::from("straight"));
                                                let mut s = state.lock().unwrap();
                                                undo_msg = set_timing(
                                                    &mut s,
                                                    &mut history,
                                    ...
                                    "humanize" => match rest.parse::<f32>() {
                                        Ok(v) if (0.0..=MAX_HUMANIZE_MS).contains(&v) => {
                                            ...
                                            .or_else(|| Some(format!("humanize = ±{}ms", v)));
                                        }
                                        _ => {
                                            undo_msg = Some(format!(
                                    ...
                                    "macro" => {
                                        ...
                                        if rest.is_empty() {
                                            ...
                                            match idx {
                                                None => {
                                                    undo_msg =
                                                        Some(format!("Bad macro id: {} (a-z)", id));
                                                }
                                ...
                                match target.as_str() {
                                    "track" => {
                                        if let Ok(tnum) = gt_input.parse::<usize>() {
                                            let tidx = tnum
                                                .saturating_sub(1)
                                                .min(s.tracks.len().saturating_sub(1));
                                            s.current_track = tidx;
                                            s.selected = 0;
                                        }
                                    }
                        ...
                        KeyCode::Enter => {
                            ...
                            if let (Some(layer), Ok(value)) = (layer, num.parse::<f32>()) {
                                ...
                                if let Some(unit) = unit {
                                    let action = Action::SetDelayUnit {
                                        unit,
                                        convert: false,
                                    };
                ...
                if key.code == KeyCode::Char('q') && mode == "normal" && submode.is_empty() {
                    ...
                    if let Some((idx, cmds)) = history.rec_stop() {
                        let mut s = state.lock().unwrap();
                        let quant = s.macros[idx].as_ref().map(|m| m.quant).unwrap_or_default();
                        let n = cmds.len();
                        ...
                        KeyCode::Char('p') => {
                            ...
                            if let Some(reg) = s.lane_register.clone() {
                                if !reg.is_empty() && !s.lane.is_empty() {
                                    ...
                                    let total =
                                        (reg.len() * times.max(1)).min(s.lane.len() - start);
                                    for i in 0..total {
                ...
                if let Some((op, opcount)) = pending_op {
                    ...
                    pending_op = None;
                    let mcount: usize = pending_count
                        .take()
                        .and_then(|s| s.parse().ok())
                        .unwrap_or(1);
                    ...
                    match key.code {
                        ...
                        KeyCode::Char(c2 @ ('f' | 't')) => {
                            pending_find =
                                Some((c2, Some(op), count, String::new(), Instant::now()));
                        }
                        _ => {
                ...
                if let KeyCode::Char(c) = key.code {
                    if c.is_ascii_digit() {
                        let prob_entry = mode == "insert"
                            && state.lock().unwrap().layer == state::BindTarget::Prob;
                        if prob_entry {
                            let value = if c == '0' {
                                100.0
                            } else {
                            ...
                            let action = Action::SetStepValue {
                                layer: state::BindTarget::Prob,
                                value,
                            };
                            exec_action(&state, &mut history, &action);
                            last_change = Some(action);
                ...
                if mode == "normal" {
                    match key.code {
                        ...
                        KeyCode::Char(c @ ('y' | 'd' | 'c')) => {
                            let opcount: usize = pending_count
                                .take()
                                .and_then(|s| s.parse().ok())
                                .unwrap_or(1);
                            ...
                            if let Some(action) = last_change.clone() {
                                ...
                                if matches!(
                                    ...
                                    Action::Op {
                                        op: Operator::Change,
                                        ..
                                    } | Action::OpTrack(Operator::Change)
                                ) {
                                    mode = String::from("insert");
                                }
                            } else {
...
mod tests {
    ...
    fn paste_step_undo() {
        let mut s = state_with_tracks(1);
        let mut h = History::new();
        s.register = Some(Register::Steps(vec![Step {
            active: true,
            note: 72,
            velocity: 90,
            ..Step::default()
    ...
    fn count_redo_runs_n_times() {
        ...
        assert_eq!(
            history_status("Undo", 3, || h.undo(&mut s)),
            "Undo ×3: Toggle step"
        );
    ...
    fn single_undo_message_has_no_multiplier() {
        ...
        assert_eq!(
            history_status("Redo", 5, || h.redo(&mut s)),
            "Redo: Toggle step"
        );
        assert_eq!(
            history_status("Redo", 1, || h.redo(&mut s)),
            "Nothing to redo"
        );
    }
    ...
    fn word_motions_navigate_runs() {
        ...
        assert_eq!(
            nav_word(steps, 16, 0, Motion::WordFwd),
            2,
            "w to first word start"
        );
        assert_eq!(
            nav_word(steps, 16, 2, Motion::WordFwd),
            8,
            "w skips within-word steps"
        );
        assert_eq!(nav_word(steps, 16, 8, Motion::WordFwd), 2, "w wraps");
        assert_eq!(
            nav_word(steps, 16, 8, Motion::WordBack),
            2,
            "b to previous word start"
        );
        assert_eq!(nav_word(steps, 16, 0, Motion::WordEnd), 3, "e to word end");
        assert_eq!(
            nav_word(steps, 16, 3, Motion::WordEnd),
            10,
            "e to next word end"
        );
    }
    ...
    fn motion_ranges_follow_vi_semantics() {
        ...
        let t = &s.tracks[0];
        assert_eq!(
            motion_range(t, 2, Motion::WordFwd, 1),
            Some((2, 7)),
            "dw eats up to next word"
        );
        assert_eq!(
            motion_range(t, 8, Motion::WordFwd, 1),
            Some((8, 15)),
            "dw at last word eats to end"
        );
        assert_eq!(
            motion_range(t, 2, Motion::WordEnd, 1),
            Some((2, 3)),
            "de is inclusive"
        );
        assert_eq!(
            motion_range(t, 8, Motion::WordBack, 1),
            Some((2, 7)),
            "db back to word start"
        );
        assert_eq!(motion_range(t, 5, Motion::End, 1), Some((5, 15)));
        assert_eq!(motion_range(t, 5, Motion::Home, 1), Some((0, 4)));
        assert_eq!(
            motion_range(t, 0, Motion::Home, 1),
            None,
            "d0 at 0 is a no-op"
        );
        assert_eq!(
            motion_range(t, 2, Motion::Till(8), 1),
            Some((2, 7)),
            "t is exclusive"
        );
        assert_eq!(
            motion_range(t, 2, Motion::Find(8), 1),
            Some((2, 8)),
            "f is inclusive"
        );
        assert_eq!(
            motion_range(t, 8, Motion::Till(2), 1),
            Some((3, 8)),
            "t works backward"
        );
        assert_eq!(
            motion_range(t, 4, Motion::Right, 3),
            Some((4, 6)),
            "3l range"
        );
    ...
    fn delete_word_clears_and_yanks() {
        ...
        assert!(
            !s.tracks[0].steps[8].active
                && !s.tracks[0].steps[9].active
                && !s.tracks[0].steps[10].active
        );
        match s.register {
    ...
    fn counted_paste_repeats_register() {
        ...
        assert!(
            s.tracks[0].steps[3].active
                && s.tracks[0].steps[4].active
                && s.tracks[0].steps[5].active
        );
    ...
    fn euclidean_short_length_deactivates_tail() {
        ...
        assert!(
            steps[8..].iter().all(|s| !s.active),
            "steps beyond length must be off"
        );
    ...
    fn fresh_session_defaults_are_playable_but_paused() {
        ...
        for ti in [0usize, 2] {
            ...
            let actives = s.tracks[ti].steps[..16]
                .iter()
                .filter(|st| st.active)
                .count();
        ...
        let lead_min = s.tracks[0].steps[..16]
            .iter()
            .filter(|st| st.active)
            .map(|st| st.note)
            .min();
        let bass_max = s.tracks[2].steps[..16]
            .iter()
            .filter(|st| st.active)
            .map(|st| st.note)
            .max();
    ...
    fn walk(mode: state::CycleMode, len: usize, n: u64) -> Vec<usize> {
        ...
        (0..n)
            .map(|g| cycle_pos(mode, g, len, &mut drunk, track_seed(0)))
            .collect()
    }
    ...
    fn cycle_forward_reverse_pingpong() {
        assert_eq!(
            walk(state::CycleMode::Forward, 4, 6),
            vec![0, 1, 2, 3, 0, 1]
        );
        assert_eq!(
            walk(state::CycleMode::Reverse, 4, 6),
            vec![3, 2, 1, 0, 3, 2]
        );
    ...
    fn cycle_exotics_stay_in_range_and_permute() {
        ...
        assert_eq!(
            walk(state::CycleMode::EveryOther, 5, 5),
            vec![0, 2, 4, 1, 3]
        );
    ...
    fn step_hz_unscaled_is_midi_and_scaled_is_degrees() {
        ...
        assert!(
            (step_hz(&trk, 127, 12) - crate::theory::scales::midi_to_hz(127)).abs() < 1e-6,
        ...
        assert!(
            (step_hz(&scaled, 60, 5) - 880.0).abs() < 1e-6,
            "bind offset moves in degrees"
        );
        assert!(
            (step_hz(&scaled, 55, 0) - 220.0).abs() < 1e-6,
            "below the root works"
        );
    }
    ...
    fn persistence_roundtrips_new_fields() {
        ...
        assert!(
            (sc.degrees[2] - 350.0).abs() < 1e-9,
            "quarter tones survive"
        );
    ...
    fn marked_tracks_fan_out_as_one_undo() {
        ...
        let _ = msg;
        assert!(
            s.tracks[0].muted && s.tracks[2].muted,
            "marked tracks muted"
        );
    ...
    fn vline_span_deletes_tracks_safely() {
        ...
        apply_selected(
            &mut s,
            &mut h,
            &Action::OpTrack(Operator::Delete),
            Some((1, 2)),
        );
        assert_eq!(s.tracks.len(), 2);
    ...
    fn switch_slot_swaps_patterns_and_undoes() {
        ...
        assert!(s.tracks[0].slots[1]
            .as_ref()
            .is_some_and(|p| p.steps[2].active));
    ...
    fn adjust_edits_the_active_layer_with_clamps() {
        ...
        apply_action(
            ...
            &mut h,
            &Action::Adjust {
                layer: state::BindTarget::Velocity,
                steps: 2,
                coarse: false,
            },
        );
    ...
    fn fill_density_hits_target_and_undoes() {
        ...
        apply_action(
            ...
            &mut h,
            &Action::Fill {
                kind: state::FillKind::Density,
                arg: 0.5,
                seed: 7,
            },
        );
        let active = s.tracks[0].steps[..16]
            .iter()
            .filter(|st| st.active)
            .count();
    ...
    fn bind_step_patches_and_unpatches() {
        ...
        assert!(
            (b.amount - 0.55).abs() < 1e-4,
            "rebind keeps the dialed amount"
        );
        // clamp
        for _ in 0..100 {
        ...
        assert!(
            s.tracks[0].steps[0].bind.is_some(),
            "undo replugs the cable"
        );
    ...
    fn multi_delete_yanks_what_it_removes() {
        ...
        match &s.register {
            ...
            other => panic!(
                "register should hold the deleted tracks, got {:?}",
                other.is_some()
            ),
        }
    }
    ...
    fn recording_captures_everything_undoable() {
        ...
        assert_eq!(
            cmds[2],
            state::MacroCmd::SetCycle {
                track: 1,
                mode: state::CycleMode::Reverse
            }
        );
        assert!(matches!(
            cmds[3],
    ...
    fn recorded_edits_replay_exactly_and_sweeps_merge() {
        ...
        assert_eq!(
            s.tracks[0].steps[2].velocity, velocity_after,
            "sweep replayed absolutely"
        );
    ...
    fn quant_boundaries_land_on_the_grid() {
        ...
        assert_eq!(
            quant_boundary(Quant::PatternEnd, 37, None),
            48,
            "falls back to bar"
        );
    }
    ...
    fn macros_and_lane_persist() {
        ...
        s.macros[1] = Some(Macro {
            cmds: vec![state::MacroCmd::SetMute {
                track: 0,
                muted: true,
            }],
        ...
        assert_eq!(
            s2.macros[2], s.macros[2],
            "all eight command shapes round-trip"
        );
...
"#;
    ...
    fn delay_frac_units_probability_and_clamp() {
        ...
        st.delay = 80.0;
        st.delay_prob = 60;
        let rolls: Vec<f64> = (0..200)
            .map(|g| delay_frac(&st, &off, 125.0, 0, g))
            .collect();
        assert!(rolls.iter().all(|&r| (0.0..=0.8 + 1e-9).contains(&r)));
        let straight = rolls.iter().filter(|&&r| r == 0.0).count();
        assert!(
            (40..=120).contains(&straight),
            "p=60 skips ~40%: {}",
            straight
        );
        let distinct: std::collections::HashSet<u64> = rolls.iter().map(|r| r.to_bits()).collect();
    ...
    fn ratchet_decay_fades_and_swells() {
        ...
        assert_eq!(
            ratchet_vel(100, 100, 3, 4),
            1,
            "full decay bottoms at 1, never 0"
        );
    }
    ...
    fn fire_offset_sums_and_clamps_to_the_window() {
        ...
        let st = Step::default();
        assert_eq!(
            fire_offset(&trk, &st, &off, 125.0, 0, 4),
            0.0,
            "straight by default"
        );
        trk.swing = 75;
        ...
        let total = fire_offset(&trk, &st, &off, 125.0, 0, 15);
        assert_eq!(
            total, MAX_DELAY_FRAC,
            "the sum clamps inside the step window"
        );
        ...
        for g in (0..50).step_by(2) {
            ...
            assert!(
                (0.0..=0.08 + 1e-9).contains(&o),
                "±10ms of a 125ms step: {}",
                o
            );
        }
    }
